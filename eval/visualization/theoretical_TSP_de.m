...
set(gca, 'Position', [pos(1)+0.09, pos(2)+0.28 , 0.236220472440945,0.332753710261639]);  % Only modify y-axis position
...
n0 = n * delta;
ni = n * (1 - delta) / d;
% Define the range of p_1 values
...
% Initialize storage for average variance of two-stage scheme
...
for i = 1:length(p2_values)
    ...
    q1 = (1-p1)/(d-1);
     % Calculate missing value variance
    ...
    var_ni = ((ni * p_star * (1 - p_star) + n0 * q2 * (1 - q2) + (n - n0 - ni) * q_star * (1 - q_star)) / ((p_star - q_star)^2))/(n^2);
        % Summarize average variance of two-stage scheme
...
subplot(1, 2, 2);
pos = get(gca, 'Position');
set(gca, 'Position', [pos(1), pos(2)+0.28 , 0.236220472440945,0.332753710261639]);  % Only modify y-axis position
epsilon = 1;

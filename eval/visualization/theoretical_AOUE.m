...
beta = 0.01;
d = 16;
alpha_range = linspace(beta, 0.5, 100000);  % Increase number of sampling points
n = 1000;

% Original OUE protocol
p_1 = 1/2;
q_1 = 1 / (exp(epsilon) + 1);
...
d = 16;
n = 1000;
epsilon_range = linspace(0.1, 3, 1000); % ε value range

% Initialize error arrays
var_before = zeros(size(epsilon_range));
var_after = zeros(size(epsilon_range));
p_1 = 1/2;
% Calculate errors for original and adjusted OUE protocols
for i = 1:length(epsilon_range)
    epsilon = epsilon_range(i);
    
    % Original perturbation probability
    ...
    % Adjusted perturbation probability
    if alpha / beta <= exp(epsilon)
        p_2 = 1;
        q_2 = 0;
    else
...
alpha = 0.1;
beta = 0.01;
d_range =16:1:1024; % Attribute domain size d range
...
% Calculate original protocol error
for i = 1:length(d_range)
    d = d_range(i);
    var_before(i) = (q_1 * (1 - q_1)) / (n * (p_1 - q_1)^2) + ((1 - p_1 - q_1)) / (d * n * (p_1 - q_1));
end

% Calculate adjusted protocol error
for i = 1:length(d_range)
    d = d_range(i);
    % Adjusted error calculation
    var_after(i) = (q_2 * (1 - q_2)) / (n * (p_2 - q_2)^2) + ((1 - p_2 - q_2)) / (d * n * (p_2 - q_2));
